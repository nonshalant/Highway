...
export const toggleFavorite = (toggle, storeDetails) => async dispatch => {
  try {
    if(toggle === true) {
      ...
      await axios.post('https://highway-client.onrender.com/favorite', { storeDetails }, config);

    }else { 
      const {storeName} = storeDetails;
      await axios.delete(`https://highway-client.onrender.com/favorite/${storeName}`)
    }
...
export const fetchFavoriteStores = () => async dispatch => {
  try {
    const response = await axios.get('https://highway-client.onrender.com/favorite');
