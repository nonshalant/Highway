...
export const handleIncrement = (event, increment) => async dispatch => {
  const response = await axios.patch(`https://highway-client.onrender/cart/update/${increment.productName}`, {event}, {withCredentials: true});
...
export const handleDecrement = (event, decrement) => async dispatch => {
  const response = await axios.patch(`https://highway-client.onrender/cart/update/${decrement.productName}`, {event}, {withCredentials: true});
...
export const addToCart = (product) => async dispatch => {
  try {
    const response = await axios.post('https://highway-client.onrender/cart', { product }, {withCredentials: true});
...
export const retrieveCart = () => async dispatch => {
  try {
    const response = await axios.get('https://highway-client.onrender/cart', { withCredentials: true });
...
export const removeItem = (productName) => async dispatch => {
  try {
    await axios.patch(`https://highway-client.onrender/cart/remove/${productName}`)
  } catch (error) {
    console.error(error);
  }
} 
...
export const sendToOrderReview = (itemsSelected) => async dispatch => { 
  try {
    ...
    await axios.post('https://highway-client.onrender/cart/order-review', {itemsSelected, itemsTotalPrice})
  } catch (error) {
    console.error(error)
  }
}
...
export const getOrderReview = () => async dispatch => {
  try {
    const response = await axios.get('https://highway-client.onrender/cart/order-review', { withCredentials: true })
