...
export const createPaymentIntentAction = (customerDetails) => async dispatch => {
    try {
        const response = await axios.post('https://highway-client.onrender.com/create-payment-intent', {customerDetails})
...
export const handlePaymentSuccess = () => async dispatch => {
    try {
        await axios.post('https://highway-client.onrender.com/payment-success', {})
    } catch (error) {
        console.error(error);
    }
} 
