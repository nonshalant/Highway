...
export const createPaymentIntentAction = (customerDetails) => async dispatch => {
    try {
        const response = await axios.post('https://highway-client-server.onrender.com:10000/create-payment-intent', {customerDetails})
...
export const handlePaymentSuccess = () => async dispatch => {
    try {
        await axios.post('https://highway-client-server.onrender.com:10000/payment-success', {})
    } catch (error) {
        console.error(error);
    }
} 
